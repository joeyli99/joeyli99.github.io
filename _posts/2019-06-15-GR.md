...
So, from the very beginning of GR, why Einstein believes that Newtonian Gravitation theory is not complete? As all of us know, it is seldom to see any gravitational phenomenon which can not be described by the law of gravity:

$$
F=G\dfrac{Mm}{r^2}
$$

So, what defect in Newton's theory inspires Einstein to think about the GR? The first opinion I want to share you is the **None of reference frame is "special"**. In special relativity, we know that no matter you are still, walking or run, all physical laws hold in your frameworks unless you are accelerating. The theory of SR is so successful but still leaves a blank: what happens in an accelerated world? 
...
It's nothing wrong if you keep going with pseudo-force and you will generate Newtonian gravitation theory think this additional field is fake(pseudo-force). But Einstein doesn't agree. Please remember: **Nothing is special**. If you abandon the stereotype of gravity, there is no reason for the special position(the true force) of gravity.
...
It isn't as trivial as in Newtonian mechanics by introducing a pseudo-force to keep 2nd theorem. The motion of particle in LOF is not forced by "gravity" or "pseudo-force" but follow the wrinkle of space-time. In next section we will introduce the "wrinkle function"
...
The interval is an invariant under the Lorentz transform. A corollary of invariant light-speed. So, what is Einstein's great assumption that leads to the GR? Einstein equivalence principle suggest that: **In LIF, the special relativity holds.**. EEP leads to a basic quantity of GR: the metric.

$$
ds^2=\eta_{\mu\nu}d\xi^\mu d\xi^v\\
=\eta_{\mu\nu}\dfrac{d\xi^\mu}{dx^\alpha}\dfrac{d\xi^\nu}{dx^\beta}dx^\alpha dx^\beta\\
...
Also take the Euclidean geometry as an example. As all of us known, the infinitesimal distance changes in the Cartesian coordinates is: $ds^2=dx^2+dy^2+dz^2$(Pythagorean theorem). This is an fundamental relation in Euclidean geometry. And in sperical coordinates the answer is also trivial $ds^2=dr^2+r^2d\theta^2+r^2\sin^2\theta d\phi^2$. So, $(1,1,1)$, $(1, r^2,r^2sin^2\theta)$ is the metric of cartesian and sperical coordinates. We can immediately find that the metric reflects the relation between two coordinate. That is why we want to research on metric in GR, metric->relations of coordinates->connections->geodesic

Back to GR, we define the dual metric quantity:

$$
g^{\alpha\beta}=\eta^{\mu\nu}\dfrac{dx^\alpha}{d\xi^\mu}\dfrac{dx^\beta}{d\xi^\nu}
$$

One can simply prove that $g^{\alpha\gamma}g_{\gamma\beta}=\delta^\alpha_\beta$.

An brilliant observation suggests:

$$
\Gamma^\mu_{\alpha\beta}=\dfrac{1}{2}g^{\mu\gamma}(g_{\gamma\alpha,\beta}+g_{\gamma\beta,\alpha}-g_{\alpha\beta,\gamma})
$$

This is a good exercise for everyone who is interested in GR. Please try to prove it. I will take some extra sections to introduce the physical means and the consequence of g.
