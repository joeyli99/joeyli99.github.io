...
So, what defect in Newton's theory inspires Einstein to think about the GR? The first opinion I want to share you is the **None of reference frame is "special"**. Image that a particle accelerate as a and is interacted by another particle, Newton's second law tell us that:

$$
ma=-G\dfrac{Mm}{r^2}
$$

Nothing special is it? However, in the frame of the particle itself, the viewer doesn't agree, she/he thinks that:"The particle does't have any acceleration, so it is on mechanical balanced!" We can rewrite the equation by introducing additional force term.
...
These are well-know result in high school. However, the pseudo-force theory is not complete.
  ...
  2. Importantly, there is no special reference. But, the reference frame of a=0 is special because the gravitation law is quite simple.

It's nothing wrong if you keep going with pseudo-force and you will generate Newtonian gravitation theory think this additional field is fake(pseudoforce). If particles are trapped by elevator and never know they are accelerated. The scientists of particles must conclude that:"Don't be nervous, we can't fell any gravitation because the God screened it for us!". But Einstein doesn't agree. Please remember: **Nothing is special**. So, Einstein suggest that: Both law of gravitation and acceleration are **fake**, the gravitation is just some intrinsic property or the "bending" of space-time. If you can't understand, you can watch this [video](https://www.youtube.com/watch?v=MTY1Kje0yLg&t=428s) for further understanding.
...
4. Einstein sum rule: the same upper index and lower index should be sum up, say: $A^\mu A_{\mu}=A^0 A_{0}+A^1 A_{1}+A^2 A_{2}+A^3 A_{3}$

5. $A_{\mu,\nu}$ is the abbreviation of $\dfrac{\partial A_{\mu}}{\partial x^\nu}$
6. $A^{,\nu}_{\mu}$ is the abbreviation of  $\dfrac{\partial A_{\mu}}{\partial x_\nu}$
...
The metric can be considered as the "unit" of distance. In America, people use miles as the unit. It is clear that 1 mile not equals to 1 km and we need to using the "metric" to convert them into same standard: 1 mile $\approx$ 1.6km. This 1.6 is nothing but the metric of American coordinate and International coordinate. This relation still valids for infiniesimal changes d*mile*=1.6 d*km*. The same thing raise in definition of infinitesimal distance: When the coordinate change a bit, what effects are there in distance? 

Take the Euclidean geometry as an example. As all of us known, the infinitesimal distance changes in the Cartesian coordinates is: $ds^2=dx^2+dy^2+dz^2$(Pythagorean theorem). This is an fundamental relation in Euclidean geometry. Well, what about sperical coordinates? The answer is also trivial because we know that $dx=dr\sin\theta\cos\phi+r\cos\theta\cos\phi d\theta-r\cos\theta\sin\phi d\phi$ and also other two relation. The answer is $ds^2=dr^2+r^2d\theta^2+r^2\sin^2\theta d\phi^2$. So, $(1, r^2,r^2sin^2\theta)$ is the metric of sperical coordinates. We can immediately find that the metric reflects the relation between two coordinate. That is why we want to research on metric in GR, metric->relations of coordinates->connections->geodesic

Also we define the dual metric quantity:
...
This is a good exercise for everyone who is interested in GR. Please try to prove it.
